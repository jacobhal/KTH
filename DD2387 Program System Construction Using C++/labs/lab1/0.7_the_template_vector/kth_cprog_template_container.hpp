...
void Vector<T>::print(){
  ...
  cout << "maxLength: " << limit << endl;  
  for(size_t i = 0; i < count && i < 10; ++i) {
   cout << "Pos " << i << ":       " << ptr[i] << endl;
  }
 }

template<typename T>
void Vector<T>::type_allowed() const{
	static_assert(is_move_constructible<T>::value, "Type is not move constructible!");
...
template<typename T>
typename Vector<T>::c_iterator Vector<T>::find(T const& t) const{
	for(auto i = begin(); i != end(); ++i) {
		if(t == *i){
			return i;
		}
...
template<typename T>
typename Vector<T>::c_iterator Vector<T>::end() const{
	return &ptr[size()];
}
...
template<typename T>
typename Vector<T>::c_iterator Vector<T>::begin() const{
	return ptr;
}
...
template<typename T>
Vector<T>::Vector(size_t t, T val){
	type_allowed();
	init(t);
