...
public class Socket {
    static final int MAXCONN = 3;
    static int n = 0; // number of active connections
    static Socket[] conn = new Socket[MAXCONN]; // array to handle all conn.
    int i = -1; // index of connection in array
    InputStream istr;
    OutputStream ostr;
	...
	synchronized(port) {
	    doClose();
	    n--;
	    conn[i] = conn[n]; // move highest element into "hole"
	    conn[n] = null;
	...
	synchronized (port) {
	    ...
	    while ((n == MAXCONN) || (acceptingConn != null)) {
		try {
		    port.wait();
		} catch (InterruptedException e) {
		}
