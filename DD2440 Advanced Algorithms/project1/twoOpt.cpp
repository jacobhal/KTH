#include <iostream>
#include <vector>
#include <cmath>
#include <tuple>
#include <limits>
#include <stack>
#include <algorithm>
#include <time.h>

struct Point {
	double a;
	double b;
	int   id;
};

struct Edge {
	int firstNode;
	int secondNode;
	int distance;

	void print() {
		std::cout << firstNode << " " << secondNode << " Distance: " << distance << std::endl; 
	}

	void print_ids() {
		std::cout << firstNode << "to " << secondNode << ", " << " Distance: " << distance << std::endl; 
	}
};

// Edge distance compare operator
bool operator< (const Edge& a, const Edge& b) {
	return a.distance < b.distance;
}

// Structure for union by rank (rank associated with depth of tree which allows us to keep the resulting tree small)
struct Subset {
	int parent;
	int rank;
};


// Function declarations
int distance(Point, Point);
std::vector<int> greedyAlgorithm();

void twoOpt();
void twoOptSwap(int&, int&);
int tourDistance(std::vector<int>);
std::vector<int> randomizeTour(std::vector<int> &tour);


// Global variables
std::vector <Point> points;
std::vector <Edge> edges;

std::vector<std::vector<int>> distMat;
std::vector<int> tour;
std::vector<int> newTour;
std::vector<int> resultTour;
const int iters = 13;
int nrOfPoints = 0;

using namespace std;
int main() {
	srand(time(NULL));
	clock_t startTime = clock();
	cin >> nrOfPoints;
	vector<int> even(nrOfPoints);
	vector<int> odd;


	for(int i = 0; i < nrOfPoints; ++i) {
		Point point;
		cin >> point.a;
		cin >> point.b;
		point.id = i;
		tour.push_back(i);
		even[nrOfPoints-i-1] = i;
		odd.push_back(i);
		points.push_back(point);
	}


	distMat.resize(nrOfPoints, vector<int>(nrOfPoints));

	for (int i = 0; i < distMat.size(); ++i) {
		for(int j = 0; j < distMat[i].size(); ++j) {
			distMat[i][j] = 0;
		}
	}

	for(int i = 0; i < nrOfPoints; ++i) {
		for(int j = i+1; j < nrOfPoints; ++j) {
			int dist = distance(points[i], points[j]);
			Edge edge;
			edge.firstNode = i;
			edge.secondNode = j;
			edge.distance = dist;
			distMat[i][j] = dist;
			distMat[j][i] = dist;
			edges.push_back(edge);
		}
	}

/*
	for (int i = 0; i < distMat.size(); ++i) {
		for(int j = 0; j < distMat[i].size(); ++j) {
			cout << distMat[i][j] << " ";
		}
		cout << endl;
	}
*/


	//vector<Point>().swap(points);
	resultTour.resize(nrOfPoints);

	newTour.resize(tour.size());
	int currentLowestCost = numeric_limits<int>::max();
	int iterLowestCost = 0;
	tour = greedyAlgorithm();
	iterLowestCost = tourDistance(tour);

	resultTour = tour;
	currentLowestCost = iterLowestCost;
	clock_t currTime;
	float timeInMs = 0.0;
	int choice = 0;
	while (timeInMs < 1.85) {
		choice = rand() % 3;
		if(choice == 0) {
			tour = randomizeTour(odd);
		} else if(choice == 1){
			tour = randomizeTour(even);
		} else {
			tour = randomizeTour(tour);
		}
		twoOpt();
		iterLowestCost = tourDistance(tour);
		if (iterLowestCost < currentLowestCost) {
			resultTour = tour;
			currentLowestCost = iterLowestCost;
		}
		currTime = clock();
		timeInMs = ((float)(currTime - startTime))/CLOCKS_PER_SEC;
	}
	
	for (int i = 0; i < resultTour.size(); ++i) {
		cout << resultTour[i] << endl;
	}

	return 0;
};

void twoOptSwap(int &i, int &j) {
    //cout << "Adding first routes\n";

	for (int k = 0;  k <= i-1; ++k)
	{
		newTour[k] = tour[k];
    	//cout << "From: " << tour[k] << " To: " << tour[k+1] << endl;

	}

	//cout << "---SWAPPING---\n";

	int decrementer = 0;
    for (int k = i; k <= j; ++k)
    {
    	//cout << "From: " << tour[k] << " To: " << tour[j-decrementer] << endl;
    	newTour[k] = tour[j-decrementer];
        decrementer++;
    }

    //cout << "Adding last routes\n";

	for (int k = j+1;  k < tour.size(); ++k)
	{
		newTour[k] = tour[k];
	}

	
}


void twoOpt() {
	int tourSize = nrOfPoints;

	int improvement = 0;
	// Compute total tour distance
	int bestDist = tourDistance(tour);
	// contains the i and j index for the best available swap.
	// i = index 0
	vector<int> bestPartTourIndex(2);
	while (improvement < 2) {
		for (int i = 0; i < tourSize - 1; ++i) {
		    for (int j = i + 1; j < tourSize; ++j) {
				int indexI = i-1;
				int indexJ = j+1;
				if (i == 0) {
					indexI = 0;
				}
				if (indexJ == tourSize) {
					indexJ = j;
				}
				// kolla alla möjliga valueswaps sen gör den som är mest value.
				int oldPartTour = distMat[tour[indexI]][tour[indexI+1]] + distMat[tour[j]][tour[indexJ]];
				int newPartTour = distMat[tour[indexI]][tour[j]] + distMat[tour[indexI+1]][tour[indexJ]];
				if (oldPartTour < newPartTour) {
		        	//bestPartTourIndex[0] = i;
		        	//bestPartTourIndex[1] = j;
		        	continue;
		        }
		        // Update newTour with performed swap
				twoOptSwap(i, j);
			    int newDist = tourDistance(newTour);
			    if (newDist < bestDist) {
			        improvement = 0;
			        tour = newTour;
			        bestDist = newDist;
			    }
		    }
		}
		improvement++;
	}
}

int tourDistance(vector<int> seq) {
	/*
	int res = 0;
	int dist = 0;
	int firstNode = seq[0];
	for(int i = 0; i < tour.size()-1; ++i) {
		dist = distance(points[seq[i]], points[seq[i+1]]);
		res += dist;
	}
	res += distance(points[seq[nrOfPoints-1]], points[firstNode]);
	return res;
	*/
	

	int res = 0;
	for (int i = 0; i < tour.size()-1; ++i)
	{
		res += distMat[seq[i]][seq[i+1]];
	}
	res += distMat[seq[nrOfPoints-1]][seq[0]];
	//cout << "SUM: " << res << endl;
	return res;
	
}
 
/**
 * @brief      A greedy algoritm choosing the shortest path at every vertice.
 *
 * @return     Return chosen path.
 */
vector<int> greedyAlgorithm() {
	// Define path vector and initialize
	vector<int> path(nrOfPoints);
	vector<bool> isVisited(nrOfPoints);
	//srand(time(0));
	//int randStart = rand() % nrOfPoints;
	path[0] = 0;
	isVisited[0] = true;

	//cout << "0: (" << points[path[0]].a << "," << points[path[0]].b << ")  \t" << endl;

	int best;
	int sum = 0;
	for(int i = 1; i < nrOfPoints; ++i) {
		best = -1;
		for(int j = 0; j < nrOfPoints; ++j) {
			
			if( (!isVisited[j] &&  best == -1) || (!isVisited[j]  && (distance(points[path[i-1]], points[j]) < distance(points[path[i-1]], points[best])) ) ) {
				best = j;
			}
		}
		path[i] = best;
		isVisited[best] = true;

	}

	return path;
}

//Compute euclidian distance between two points (round to nearest integer)
int distance(Point firstPoint, Point secondPoint) {
	return sqrt(pow((secondPoint.a - firstPoint.a), 2) + pow((secondPoint.b - firstPoint.b), 2)) + 0.5;
}
// randomizes a new tour
vector<int> randomizeTour(vector<int> &tour) {
	//srand(time(NULL));
	random_shuffle(tour.begin(), tour.end());
	return tour;
	//srand(time(NULL));
	//random_shuffle(tour.begin(), tour.end());
}