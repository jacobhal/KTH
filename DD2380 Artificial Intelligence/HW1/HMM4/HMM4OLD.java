			...
			*/
			for (int t = 1; t < nrOfEmissions; t++) {
				for (int i = 0; i < transColumnSize; i++) {
					for (int j = 0; j < transColumnSize; j++) {
						//System.out.println(alphaMatrix.get(t-1).getValue(j, 0));
					}
				}
			}
			// compute alphat(i)
			...
			for (int t = 1; t < nrOfEmissions; t++) {
				...
				for (int i = 0; i < transColumnSize; i++) {
					...
					for (int j = 0; j < transColumnSize; j++) {
						...
						//System.out.println("j: " + j + ", t-1: " + (t-1) + ", alpha: " + prevAlpha);
			...
			//for (int i = 0; i < initBeta.getRowLength(); i++) {
			//	System.out.println("beta[" + i + "]: " + initBeta.getValue(i, 0));
			//}
			...
			for (int t = nrOfEmissions-2; t >= 0; t--) {
				...
				betaPart = betaPart.scalarMultiply(cScaleVector[t]);
			...
			for (int t = 0; t < nrOfEmissions-1; t++) {
				...
				for(int i = 0; i < transColumnSize; i++) {
					for(int j = 0; j < transColumnSize; j++) {
						double alphaT = alphaMatrix.get(t).getValue(i, 0);
						double aij = transMatrix.getValue(i, j);
						double bJ = emissMatrix.getValue(j, emissSequence[t+1]);
						double nextBt = betaMatrix.get(t+1).getValue(j, 0);
						part = alphaT * aij * bJ * nextBt;
						denom = denom + part;
