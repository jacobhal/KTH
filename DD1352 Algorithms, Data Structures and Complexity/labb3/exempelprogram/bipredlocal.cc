...
std::multimap<int, int> readBipartiteGraph() {
...
void writeFlowGraph(std::multimap<int, int> edges) {
  /*
  ...
  // Måste vara på detta format för att maxflow ska acceptera källa o sänka.
  source = 1;
  target = 2;
  ...
  for(int i = 0; i < gX; ++i) {
    if(i <= gX) {
      ...
      edges.insert(std::pair<int, int>(i, target));
    }
  }
...
int main(void) {
  ...
  edges = readBipartiteGraph(); // Läser in bipartit graf

  writeFlowGraph(edges); // 
