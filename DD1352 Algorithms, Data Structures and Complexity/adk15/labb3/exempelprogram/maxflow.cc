...
struct Edge {
  ...
  bool operator==(const Edge & source) {
  	return (this->v1 == source.v1) && (this->v2 == source.v2) && (this->edgeRestCapacity == source.edgeRestCapacity);
  }
  ...
  */
  ...
  void print() {
  	cout << this->v1 << " " << this->v2 << " " << this->edgeCapacity << "\n";
  }
};
...
std::vector<int> neighbourCapacities;
...
void fillNeighbours() {
	...
	for(int i = 0; i < edges.size(); ++i) {
		...
		*/
		...
		// Neighbours to current edge
		n = current.neighbours(edges);
		/*
		cout << "Neighbours: ";
		for(int j = 0; j < n.size(); ++j) {
			cout << n[j].v1 << " " << n[j].v2 << " " << n[j].edgeCapacity << "   ";
		}*/


		// Store vector n at current.v1 value
		//neighbours[current.v2].insert(neighbours[current.v2].end(), n.begin(), n.end());
		neighbours[current.v2] = n;
		//cout << "\n";
	}
	n = Edge(0, fSource, 0).neighbours(edges);
    ...
    while(!q.empty()) {
 	...
 	/*	
 		for(int i = 0; i < n.size(); ++i) {
 			if(n[i] == nb[i]) {
 				cout << "n[i]: " << n[i].v1 << " " << n[i].v2 << " " << n[i].edgeCapacity << " " << n[i].edgeFlow << "/" << n[i].edgeRestCapacity << "\n";
 				cout << "nb[i]: " << nb[i].v1 << " " << nb[i].v2 << " " << nb[i].edgeCapacity << " " << nb[i].edgeFlow << "/" << nb[i].edgeRestCapacity << "\n";
 			} else {
 				cout << "FEL" << "\n";
 				cout << "n[i]: " << n[i].v1 << " " << n[i].v2 << " " << n[i].edgeCapacity << " " << n[i].edgeFlow << "/" << n[i].edgeRestCapacity << "\n";
 				cout << "nb[i]: " << nb[i].v1 << " " << nb[i].v2 << " " << nb[i].edgeCapacity << " " << nb[i].edgeFlow << "/" << nb[i].edgeRestCapacity << "\n";
 				cout << "n[i].edgeRestCapacity: " << n[i].edgeRestCapacity << " " << "nb[i].edgeRestCapacity: " << nb[i].edgeRestCapacity << "\n";
 			}
 		}
 	*/	
...
/*
  for(int i = 0; i < edges.size(); ++i) {
  	...
  	cout << neighbours[i][j].v1 << " " << neighbours[i][j].v2 << " " << neighbours[i][j].edgeCapacity << "   ";
	  }
	  cout << "\n";
...
// Debugutskrift
	...
	for(int i = edges.size()-1; i >= 0; --i) {
		...
		if(edges[i].exists(tmpPath)) {
			...
			//neighbourCapacities[i] = edges[i].edgeRestCapacity;
			/*
			...
			*/
			/*
			for(int j = 0; j < neighbours[fSource].size(); ++j) {
				
				if(neighbours[fSource][j].exists(tmpPath)) {
					cout << "Härevi" << "\n";
					neighbours[fSource][j].edgeFlow += path_flow;
					neighbours[fSource][j].edgeRestCapacity = neighbours[fSource][j].edgeCapacity - neighbours[fSource][j].edgeFlow;
				}
			}*/
