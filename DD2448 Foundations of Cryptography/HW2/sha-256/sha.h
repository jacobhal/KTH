...
// Buffer to hold 4 bytes (one word)
typedef unsigned int word;


// These words represent the first thirty-two bits of the fractional parts of the cube roots of the first sixtyfour
// prime numbers. 
const word k_value[64] = 
...
// Convert a string to a word vector and begin padding. Return true if partial padding was made.
bool string_to_wordvector(vector<word>& block, string s);

// Convert byte to hex string
string to_hex(word w);

// Convert a string to hexadecimal string (used for testing only)
...
void pad(vector<word>& message, int inputsize, bool padded);
