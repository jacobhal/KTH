...
struct BLOCK_MAT {
  ...
  // Print as hex string on one line
  void printHex() {
  ...
  // Print result in same format as output
  void printRes() {
    for(int i = 0; i < MAT_SIZE; ++i) {
      for(int j = 0; j < MAT_SIZE; ++j) {
        cout << matrix[j][i];
...
void subBytes(BLOCK_MAT&);

void shiftRows(BLOCK_MAT&);

void mixColumns(BLOCK_MAT&);
