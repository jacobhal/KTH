...
// Store byte matrices here, first block will contain the key
vector<BLOCK> blocks;
// Store round keys here
vector<unsigned char*> expandedKeys;
// Buffer to hold 8 bits (1 byte)
unsigned char byte;
// Byte matrix
BLOCK block(MAT_SIZE, MAT_SIZE); 
...
int main() {
  ...
  while(cin >> byte) {
    ...
    // Go to next row if we are at last column
    if(col == 4) {
      col = 0;
      row++;
    }
...
// Rotate word one step to the left
void rotWord(int row, BLOCK &block) {
  unsigned char tmp = block.getVal(row, 0);
  for(int i = 0; i < MAT_SIZE-1; ++i) {
    block.setVal(row, i, block.getVal(row, i+1));
  }
  block.setVal(row, 3, tmp);
}
...
void subWord(int row, BLOCK &block) {
